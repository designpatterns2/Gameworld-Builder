...
public class MapBuilderImplementation implements MapBuilder {

    private static final Logger logger = Logger.getLogger(MapBuilderImplementation.class.getName());
    ...
    public MapBuilderImplementation(Random random) {
        this.random = random;
    }
     ...
     */
    private MapBuilderImplementation.Territory getClosestTerritory(final Tile tile,
                                                             List<MapBuilderImplementation.Territory> territories) {
        final Map map = tile.getMap();
        final Comparator<MapBuilderImplementation.Territory> comp = Comparator.comparingInt(t ->
     ...
     *     none placed.
     */
    private IndianSettlement placeCapital(final Map map, MapBuilderImplementation.Territory territory,
    ...
    private List<Map.Position> generateStartingPositions(Map map,
                                                         List<Player> players) {
        final Specification spec = map.getGame().getSpecification();
        int number = players.size();
        List<Map.Position> positions = new ArrayList<>(number);
        if (number > 0) {
            int west = 0;
            ...
            switch (spec.getInteger(GameOptions.STARTING_POSITIONS)) {
                ...
                case GameOptions.STARTING_POSITIONS_HISTORICAL:
                    for (Player player : players) {
                        ...
                        positions.add(new Map.Position(where,
    ...
    @Override
    public LandMap buildLandMap(Game game, Map importMap, LogBuilder lb) {
    ...
    @Override
    public Map buildTerrain(Game game, Map importMap, LandMap landMap, LogBuilder lb) {
     ...
     * @param lb A {@code LogBuilder} to log to.
     */
    public void buildEuropeanUnits(Map map, List<Player> players,
                                    LogBuilder lb) {
        final Game game = map.getGame();
        final Specification spec = game.getSpecification();
        ...
        List<Map.Position> positions = generateStartingPositions(map, europeanPlayers);
     ...
     * @param lb A {@code LogBuilder} to log to.
     */
    public void buildLostCityRumours(Map map, Map importMap, LogBuilder lb) {
        final Game game = map.getGame();
     ...
     * @param lb A {@code LogBuilder} to log to.
     */
    public void buildNativeSettlements(final Map map, Map importMap,
                                        LogBuilder lb) {
        final Game game = map.getGame();
        final Specification spec = game.getSpecification();
        ...
        HashMap<String, MapBuilderImplementation.Territory> territoryMap = new HashMap<>();
        ...
        for (Player player : players) {
            ...
            MapBuilderImplementation.Territory territory = null;
            if (regionKeys == null || regionKeys.isEmpty()) {
                territory = new MapBuilderImplementation.Territory(player, map.getRandomLandTile(random));
                ...
                for (String key : regionKeys) {
                    if (territoryMap.get(key) == null) {
                        ...
                        if (region == null) {
                            ...
                            territory = new MapBuilderImplementation.Territory(player, region);
                        }
                ...
                if (territory == null) {
                    ...
                    outer: for (String key : regionKeys) {
                        MapBuilderImplementation.Territory otherTerritory = territoryMap.get(key);
                        for (String otherKey : ((IndianNationType) otherTerritory.player.getNationType())
                            ...
                            if (territoryMap.get(otherKey) == null) {
                                ...
                                territory = new MapBuilderImplementation.Territory(player, foundRegion);
                                territoryMap.put(key, territory);
                                break outer;
                            }
                        }
        ...
        for (MapBuilderImplementation.Territory territory : territories) {
        ...
        while (!settlementTiles.isEmpty() && !territories.isEmpty()) {
            ...
            MapBuilderImplementation.Territory territory = getClosestTerritory(tile, territories);
